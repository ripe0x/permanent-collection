...
```bash
cast call {{addr:patron}} "bidBalance()(uint256)" \
...
BID=$(cast call {{addr:patron}} "bidBalance()(uint256)" --rpc-url $RPC)

# 2. Read the protocol-derived canonical target for the Punk
PC=$(cast call {{addr:patron}} "permanentCollection()(address)" --rpc-url $RPC)
...
cast send {{addr:patron}} "acceptBid(uint16,uint8,uint256)" \
  $PUNK_ID $TARGET $BID \
...
## function allowedSellerActiveAt
