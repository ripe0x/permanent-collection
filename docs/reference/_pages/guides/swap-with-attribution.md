---
title: Swap with referral attribution
description: How to attach referral attribution to a $111 swap via V4 hookData, get credited in ReferralPayout on every swap, and claim the accrued ETH.
---
...
2. pays the slice into [ReferralPayout](/docs/contracts/referral-payout) at [{{addr:referralPayout}}](https://evm.now/address/{{addr:referralPayout}}?chainId=1) via its hook-only `notify(referrer)`, which increments `balances[referrer]` and emits `ReferralCredited(referrer, amount)`
...
```bash
cast send {{addr:referralPayout}} "claim()" \
