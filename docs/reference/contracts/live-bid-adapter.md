...
`0x8C72FBc2bB32e76aa54243F76745266a0F92CD01` · [view on evm.now](https://evm.now/address/0x8C72FBc2bB32e76aa54243F76745266a0F92CD01?chainId=1) · deployed at block 25270161
...
Deployed at [0x8C72FBc2bB32e76aa54243F76745266a0F92CD01](https://evm.now/address/0x8C72FBc2bB32e76aa54243F76745266a0F92CD01?chainId=1).
...
cast call 0x8C72FBc2bB32e76aa54243F76745266a0F92CD01 "bufferedEth()(uint256)" \
  --rpc-url https://ethereum-rpc.publicnode.com

# first block at which the next THROTTLED forward is allowed
cast call 0x8C72FBc2bB32e76aa54243F76745266a0F92CD01 "nextSweepBlock()(uint256)" \
  --rpc-url https://ethereum-rpc.publicnode.com

# the fast/throttled boundary and the current throttle parameters
cast call 0x8C72FBc2bB32e76aa54243F76745266a0F92CD01 "activationThreshold()(uint256)" \
  --rpc-url https://ethereum-rpc.publicnode.com
cast call 0x8C72FBc2bB32e76aa54243F76745266a0F92CD01 "maxSweepWei()(uint256)" \
  --rpc-url https://ethereum-rpc.publicnode.com
cast call 0x8C72FBc2bB32e76aa54243F76745266a0F92CD01 "minBlocksBetweenSweeps()(uint256)" \
  --rpc-url https://ethereum-rpc.publicnode.com

# Patron's balance decides the mode: below activationThreshold = fast mode
cast balance $(cast call 0x8C72FBc2bB32e76aa54243F76745266a0F92CD01 "patron()(address)" \
  --rpc-url https://ethereum-rpc.publicnode.com) \
  --rpc-url https://ethereum-rpc.publicnode.com
```
...
```solidity
function streamForward() external returns (uint256 ethForwarded)
```
...
const hash = await wallet.writeContract({
  address: '0x8C72FBc2bB32e76aa54243F76745266a0F92CD01',
  abi,
...
```solidity
function poolReplenish(uint16 punkId) external payable
```
...
```solidity
function setMaxSweepWei(uint256 newValue) external
```
...
### setMinBlocksBetweenSweeps

```solidity
function setMinBlocksBetweenSweeps(uint256 newValue) external
```
...
```solidity
function setActivationThreshold(uint256 newValue) external
```
...
```solidity
function ACTIVATION_THRESHOLD_HI() external view returns (uint256)
```

Upper bound on `activationThreshold`: 100 ETH (`100e18`). Both the setter and
the auto-sync clamp to it.

### ACTIVATION_THRESHOLD_LO

```solidity
function ACTIVATION_THRESHOLD_LO() external view returns (uint256)
```
...
```solidity
function KEEPER_REWARD_BPS() external view returns (uint256)
```
...
```solidity
function KEEPER_REWARD_CAP() external view returns (uint256)
```
...
### lastSyncedAcquisitionCount

```solidity
function lastSyncedAcquisitionCount() external view returns (uint256)
```
...
```solidity
function MAX_SWEEP_WEI_HI() external view returns (uint256)
```
...
```solidity
function MAX_SWEEP_WEI_LO() external view returns (uint256)
```
...
```solidity
function maxSweepWei() external view returns (uint256)
```
...
```solidity
function MIN_STREAM_WEI() external view returns (uint256)
```
...
### minBlocksBetweenSweeps

```solidity
function minBlocksBetweenSweeps() external view returns (uint256)
```
...
```solidity
function permanentCollection() external view returns (IPCAcquisitionReader)
```
...
### REFERRER_CONTRIB_BPS

```solidity
function REFERRER_CONTRIB_BPS() external view returns (uint256)
```
...
**`SweepTooEarly(uint256 nextBlock)`**
