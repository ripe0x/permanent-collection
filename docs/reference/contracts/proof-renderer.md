...
`cast call 0x40aa9edb0063ca5d12eef8A53be45F7e15a3Fe10 "proofRenderer()(address)" --rpc-url
...
```solidity
function PROOF_COUNT() external view returns (uint256)
```
...
```solidity
function svg(uint8 traitId) external view returns (string)
```
...
```solidity
function vault() external view returns (IPunkVaultProofView)
```
